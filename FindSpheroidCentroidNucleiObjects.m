...
% Filter connected components based on area
selection = (rprops_areas > 10) & (rprops_areas < 300);
...
caption = sprintf('Outlines of only blobs 10-300 px^2.\nBlobs are numbered from top to bottom, then from left to right.');
title(caption, 'FontSize', captionFontSize);
axis('on', 'image'); % Make sure image is not artificially stretched because of screen's aspect ratio.
...
% 	text(blobCentroid(k,1), blobCentroid(k,2), num2str(k), 'FontSize', 8, 'FontWeight', 'Bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'Color','y');
% 
% end
...
sgtitle('Outlines, from bwboundaries()', 'FontSize', captionFontSize);
...
%------------------------------------------------------------------------------------------------------------------------------------------------------
%% An alternative guide to potentially filtering the nuclei
...
% allBlobIntensities = [props_f.MeanIntensity];
% allBlobAreas = [props_f.Area];
% figure
...
% %------------------------------------------------------------------------------------------------------------------------------------------------------
% % Plot the centroids in the overlay above the original image in the upper
% % left axes. With either a red + or blue x
...
%  % Save the matlab figures ...................    
%     figHandles = flip(findall(0,'Type','figure'),1);
% 
%     % Create a new folder with a unique name
%     newFolderName = ['Binarization and Labelling of Image', num2str(f)];
%     newFolderPath = fullfile(selected_folder, newFolderName);
%     mkdir(newFolderPath);
% 
%     % Save each figure with a unique file name in the specified folder
%     for i = 1:numel(figHandles)  
%         % Specify the file name for the saved figure
%         fileName = ['Fig' num2str(i) ' quantified image' num2str(f)];
% 
