function [outer_distance_magnitude, outer_distances_xy, full_distance_magnitude, full_distances_xy, ...
    boundary_intersect, horizontal_line] = CalculateDistancesBlobs(outer_centroids, centroid_loc, boundary, binary_image)
...
full_distances_xy(:,1) = outer_centroids(:,1) - centroid_loc(1);
full_distances_xy(:,2) = outer_centroids(:,2) - centroid_loc(2);

%Distance formula: distances = sqrt((x - x_center).^2 + (y - y_center).^2);
full_distance_magnitude = sqrt( (outer_centroids(:,2)-centroid_loc(:,2)).^2 + (outer_centroids(:,1)-centroid_loc(:,1)).^2 );

% full_distance_magnitude = zeros(length(outer_centroids),1);
% full_distances_xy = zeros(length(outer_centroids),1);
% for i = 1:length(outer_centroids) 
%     full_distances_xy{i}(:,1) = outer_centroids{i}(:,1) - centroid_loc(1);
%     full_distances_xy{i}(:,2) = outer_centroids{i}(:,2) - centroid_loc(2);
% 
%     %Distance formula: distances = sqrt((x - x_center).^2 + (y - y_center).^2);
%     full_distance_magnitude{i} = sqrt( (outer_centroids{i}(:,2)-centroid_loc(:,2)).^2 + (outer_centroids{i}(:,1)-centroid_loc(:,1)).^2 );
% 
% end
...
for i = 1:length(outer_centroids)
        ...
        plot([centroid_loc(1), outer_centroids(i,1)], [centroid_loc(2), outer_centroids(i,2)], 'b')
...
[outer_distance_magnitude, boundary_xint,boundary_yint] = IntersectionDistance(outer_centroids, centroid_loc, boundary);
%outer_distance_magnitude{i,1} = od_magnitude;
boundary_intersect = [boundary_xint boundary_yint];

%Calculate x-y- distances to boundary
outer_distances_xy(:,1) = outer_centroids(:,1) - boundary_intersect(:,1);
outer_distances_xy(:,2) = outer_centroids(:,2) - boundary_intersect(:,2);
...
% outer_distance_magnitude = cell(length(outer_centroids), 1);
% boundary_intersect = cell(length(outer_centroids), 1);
% outer_distances_xy = cell(length(outer_centroids),1);
% 
% for i = 1:length(outer_centroids)
% 
%     %Calculate distance magnitude to boundary
%     [od_magnitude, boundary_xint,boundary_yint] = IntersectionDistance(outer_centroids{i}, centroid_loc, boundary);
%     outer_distance_magnitude{i,1} = od_magnitude;
%     boundary_intersect{i} = [boundary_xint boundary_yint];
% 
%     %Calculate x-y- distances to boundary
%     outer_distances_xy{i}(:,1) = outer_centroids{i}(:,1) - boundary_intersect{i}(:,1);
%     outer_distances_xy{i}(:,2) = outer_centroids{i}(:,2) - boundary_intersect{i}(:,2);
% 
% end
%time_calc = toc
...
for i = 1:length(outer_centroids)
    ...
    plot([boundary_intersect(i,1), outer_centroids(i,1)], [boundary_intersect(i,2), outer_centroids(i,2)], 'c')
